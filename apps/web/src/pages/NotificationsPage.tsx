  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold text-gray-900 dark:text-white mb-4 flex items-center gap-2">
        <Bell className="w-5 h-5" /> {t('notifications.title', 'Уведомления')}
      </h1>
      {isLoading ? <div className="flex justify-center py-12"><Spinner /></div> : notifications.length === 0 ? (
        <div className="text-center py-12 text-gray-500">{t('notifications.empty', 'Нет уведомлений')}</div>
      ) : (
        <div className="space-y-2">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between">
                        ...
                        <Badge variant={n.type === 'NEW_COLLECTION' ? 'info' : n.type === 'RE_NOTIFY' ? 'warning' : 'default'}>
                          {n.type === 'NEW_COLLECTION' ? t('notifications.new', 'Новый') : n.type}
                        </Badge>
                      </div>
                      <p className="text-sm text-gray-600 dark:text-gray-400">{n.collection?.amount} {n.collection?.currency}</p>
                      {path.length > 0 && (
                        <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                          ...
                          <span className="ml-1">{path.length} {t('notifications.handshakes', 'рукопожатий')}</span>
                        </p>
                      )}
                      <div className="flex gap-2 mt-2">
                        <Button size="sm" onClick={() => { markRead.mutate({ id: n.id }); navigate(`/collection/${n.collectionId}`); }}>
                          <Check className="w-3 h-3 mr-1" /> {t('notifications.view', 'Открыть')}
                        </Button>
                        <Button size="sm" variant="ghost" onClick={() => dismiss.mutate({ id: n.id })}>
                          <X className="w-3 h-3 mr-1" /> {t('notifications.dismiss', 'Закрыть')}
                        </Button>
                      </div>
                    </div>
                  </div>
          ...
          {hasNextPage && <Button variant="outline" className="w-full" onClick={() => fetchNextPage()}>{t('notifications.loadMore', 'Загрузить ещё')}</Button>}
        </div>
      )}
    </div>
  );
