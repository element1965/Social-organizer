...
Организер сразу и одновременно уведомляет количество людей, равное **сумме запроса**. Соотношение 1:1.

Пример:
- сумма запроса: 300
- значит уведомляется **300 человек**
...
> **Уточнение:** Если в цепочке связей доступно меньше людей, чем сумма сбора — сбор всё равно создаётся, уведомляются все доступные. Далее действует механизм доуведомления (см. 2.7).
...
> - Количество доуведомлений = оставшаяся сумма (1:1).
> - Пример: запрос 300, за 12ч собрано 200 → отправляется ещё **100 уведомлений** (300−200).
