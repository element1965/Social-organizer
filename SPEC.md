...
>
> **Валюты:** Фиксированный список **только фиатных** валют (USD, EUR, RUB, UAH, GBP, PLN, ...). Криптовалюты не упоминаются в списке. Пользователь выбирает из списка, произвольный ввод невозможен.
>
...
При удалении:
- **Все данные пользователя удаляются** полностью (ФИО, фото, телефон, био, история, статистика).
- **Связи освобождаются** — у каждого, кто был связан с удалённым, появляется свободный слот для нового приглашения.
- Исторические записи об участии в сборах **анонимизируются** (запись остаётся как «удалённый пользователь», чтобы не ломать статистику сборов).
...
> Это единственный случай, когда связь может быть «убрана» из 150.
