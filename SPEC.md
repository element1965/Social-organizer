...
Организер сразу и одновременно уведомляет количество людей, равное **сумме запроса / 10** (т.к. минимальное обязательство — 10 единиц). Соотношение 1:10.

Пример:
- сумма запроса: 5 000
- значит уведомляется **500 человек** (5 000 / 10)
...
> **Уточнение:** Если в цепочке связей доступно меньше людей, чем сумма/10 — сбор всё равно создаётся, уведомляются все доступные. Далее действует механизм доуведомления (см. 2.7).
...
> - Количество доуведомлений = (оставшаяся сумма) / 10.
> - Пример: запрос 5 000, за 12ч собрано 4 000 → отправляется ещё **100 уведомлений** ((5000−4000) / 10).
...
## Регистрация и рост сети

> **Ключевой принцип: открытая регистрация + органическое слияние сетей.**
>
> Любой человек может **самостоятельно скачать приложение и зарегистрироваться** — приглашение НЕ обязательно. После регистрации он начинает строить свою сеть: приглашает друзей, они приглашают своих.
>
> Изначально это **изолированная мини-сеть**. Но как только один из её участников добавит связь с пользователем из уже существующей сети — **сети сливаются**. BFS-обход теперь проходит через точку пересечения, и уведомления о сборах достигают участников обеих сетей.
>
> Это обеспечивает **органический рост**: не нужен центральный «рекрутер» или обязательная цепочка приглашений. Каждый пользователь — точка входа. Глобальная сеть растёт через слияние множества независимых мини-сетей.
>
> **Пример:** Алиса скачала приложение, пригласила 5 друзей. Борис скачал независимо, пригласил 3 друзей. Два изолированных графа. Потом один из друзей Алисы добавил одного из друзей Бориса в связи → две сети слились в одну.
>
> **Слияние через реферальную ссылку между существующими пользователями:**
> Два человека, **уже зарегистрированных** в разных изолированных сетях, могут установить связь стандартным способом — через **реферальную ссылку** (или QR-код). Один генерирует ссылку, второй её принимает → между ними создаётся связь → их сети объединяются. Для этого **не нужно** быть новым пользователем — реферальная ссылка работает как универсальный механизм добавления связи.
>
> **Пример кросс-платформенного слияния:** Пользователь A скачал приложение из Play Market, Пользователь B — из App Store. Каждый построил свою сеть. Однажды участник сети A и участник сети B узнают, что оба используют приложение. Один отправляет другому реферальную ссылку → связь создана → сети A и B слились. Все уведомления о сборах теперь проходят через точку пересечения.
...
1. **Автоматический вход** через платформу. Имя и фото подтягиваются автоматически. Приглашение не требуется — регистрация открыта для всех.
...
- **Телефон** — виден всем пользователям системы.
...
> - **Регистрация открытая** — приглашение НЕ обязательно. Любой может скачать/открыть приложение и зарегистрироваться самостоятельно.
> - Пользователь добавляет связи через: **пригласительные ссылки**, **QR-код**, **выбор друзей платформы** (FB `chooseAsync`, Telegram контакты).
> - Как только участник одной мини-сети добавит связь с участником другой — сети **сливаются** через общий граф (см. «Регистрация и рост сети»).
...
1. **Сбор без целевой суммы.** Создатель не указывает сумму при создании. Обычные пользователи обязаны указывать целевую сумму, Автор и Разработчик — нет. Участники вносят обязательства как обычно (минимум 10 единиц).

2. **Уведомления не рассылаются при создании сбора.** В отличие от обычных сборов, BFS-рассылка при создании не происходит. Вместо этого уведомление приходит **каждому пользователю индивидуально и однократно** — после того, как он **впервые поучаствовал** в каком-либо сборе (взял обязательство). Это гарантирует, что человек уже понимает систему и осознанно решает. Уведомление ничем не отличается от обычного — можно проигнорировать / закрыть.

> Это модель поддержки проекта. Участие добровольное, как и во всех сборах. Уведомление приходит только тем, кто уже участвовал в чужих сборах — т.е. уже проявил готовность помогать.
